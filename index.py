...
def submitForm(formWid, address, collectWid, schoolTaskWid, form, session, host):
    ...
    # 默认正常的提交参数json
    params = {"formWid": formWid, "address": address, "collectWid": collectWid, "schoolTaskWid": schoolTaskWid, "uaIsCpadaily": True,
              "form": form}
    # print(params)
...
def sendEmail(send,msg):
    ...
    try:
        ...
        msg['From']=formataddr(["自动打卡脚本", my_sender])  # 括号里的对应发件人邮箱昵称、发件人邮箱账号
...
你好：
    ...
    来自服务器的消息：
                      ...
                      自动提交成功！
                    '''
                    InfoSubmit(msg, user['user']['email'])
                elif msg == '该收集已填写无需再次填写':
                    log('你今天已经提交过啦，无需再次提交！')
                    # 下面的msg邮件消息内容可自行更改
                    msg = '''
